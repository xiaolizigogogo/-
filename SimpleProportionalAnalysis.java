...
public class SimpleProportionalAnalysis {
    ...
    public static void main(String[] args) {
        ...
        System.out.printf("推荐公式: 中介单价 = %.4f × 国信达单价%n", 平均比例);
        ...
        // 演示公式使用
        demonstrateFormula(平均比例);
        
        // 保存结果
        saveResultsToFile(平均比例, r2, mae, 比例标准差);
    }
    ...
    // 保存结果到文件
    private static void saveResultsToFile(double 比例系数, double r2, double mae, double 比例标准差) {
        try (PrintWriter writer = new PrintWriter(new FileWriter("proportional_formula.txt"))) {
            writer.println("=== 基于比例系数的房价调整公式 ===");
            writer.println();
            writer.println("推荐公式:");
            ...
            writer.printf("比例系数标准差 = %.4f%n", 比例标准差);
            writer.println();
            writer.println("使用说明:");
            writer.println("1. 将国信达单价乘以比例系数");
            writer.println("2. 得到调整后的中介单价预测值");
            writer.println("3. 此公式适用于所有价格区间");
            
            System.out.println("\n结果已保存到 proportional_formula.txt");
        } catch (IOException e) {
