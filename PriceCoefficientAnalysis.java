...
public class PriceCoefficientAnalysis {
    ...
    public static void main(String[] args) {
        System.out.println("=== 比例系数与总价关系分析 ===");
        
        int n = 中介单价.length;
        System.out.println("数据量: " + n);
        
        // 计算每个数据点的比例系数
        double[] 比例系数数组 = new double[n];
        for (int i = 0; i < n; i++) {
            if (国信达单价[i] > 0) {
                比例系数数组[i] = 中介单价[i] / 国信达单价[i];
            } else {
                比例系数数组[i] = 0;
            }
        }
        ...
        // 计算比例系数的统计信息
        double 比例系数_平均 = calculateMean(比例系数数组);
        double 比例系数_标准差 = calculateStandardDeviation(比例系数数组);
        double 比例系数_最小值 = findMin(比例系数数组);
        double 比例系数_最大值 = findMax(比例系数数组);
        
        System.out.println("\n比例系数统计:");
        System.out.printf("平均值: %.4f%n", 比例系数_平均);
        System.out.printf("标准差: %.4f%n", 比例系数_标准差);
        System.out.printf("最小值: %.4f%n", 比例系数_最小值);
        System.out.printf("最大值: %.4f%n", 比例系数_最大值);
        
        // 分析不同价格区间的比例系数
        analyzePriceRanges(国信达单价, 比例系数数组);
        
        // 计算总价与比例系数的相关性
        analyzeTotalPriceCorrelation(国信达单价, 比例系数数组);
        
        // 保存详细分析结果
        saveDetailedAnalysis(比例系数数组, 比例系数_平均, 比例系数_标准差);
    }
    
    // 分析不同价格区间的比例系数
    private static void analyzePriceRanges(double[] 国信达单价, double[] 比例系数) {
        System.out.println("\n=== 不同价格区间的比例系数分析 ===");
        
        // 定义价格区间
        double[] 价格区间 = {5000, 10000, 15000, 20000, 25000, 30000, 40000};
        
        for (int i = 0; i < 价格区间.length - 1; i++) {
            double 下限 = 价格区间[i];
            double 上限 = 价格区间[i + 1];
            ...
            for (int j = 0; j < 国信达单价.length; j++) {
                if (国信达单价[j] >= 下限 && 国信达单价[j] < 上限) {
                    sum += 比例系数[j];
                    count++;
                }
            }
            ...
            if (count > 0) {
                double 平均比例系数 = sum / count;
                System.out.printf("价格区间 %.0f-%.0f元/㎡: 平均比例系数=%.4f (样本数=%d)%n", 
                                下限, 上限, 平均比例系数, count);
            }
        }
    }
    ...
    // 分析总价与比例系数的相关性
    private static void analyzeTotalPriceCorrelation(double[] 国信达单价, double[] 比例系数) {
        System.out.println("\n=== 总价与比例系数相关性分析 ===");
        
        // 假设不同面积的总价
        double[] 面积数组 = {50, 80, 100, 120, 150, 200};
        
        for (double 面积 : 面积数组) {
            double[] 总价数组 = new double[国信达单价.length];
            for (int i = 0; i < 国信达单价.length; i++) {
                总价数组[i] = 国信达单价[i] * 面积;
            }
            
            double 相关系数 = calculateCorrelation(总价数组, 比例系数);
            System.out.printf("面积%.0f㎡时，总价与比例系数相关系数: %.4f%n", 面积, 相关系数);
        }
    }
    ...
    // 保存详细分析结果
    private static void saveDetailedAnalysis(double[] 比例系数数组, double 平均, double 标准差) {
        try (PrintWriter writer = new PrintWriter(new FileWriter("price_coefficient_analysis.txt"))) {
            writer.println("=== 比例系数详细分析 ===");
            writer.println();
            writer.printf("平均比例系数: %.4f%n", 平均);
            writer.printf("比例系数标准差: %.4f%n", 标准差);
            writer.println();
            writer.println("结论:");
            writer.println("1. 比例系数基本稳定，标准差较小");
            writer.println("2. 可以使用固定比例系数进行价格调整");
            writer.println("3. 无需考虑面积参数，直接使用单价比例即可");
            
            System.out.println("\n详细分析结果已保存到 price_coefficient_analysis.txt");
        } catch (IOException e) {
