/*
 * Mega2560 + ESP32 从设备 - 基于Main(2).ino的完整业务逻辑
 * 功能：保持原始Main功能 + 接收主设备数据 + 操作后反馈数据
 * 架构：Mega2560(业务逻辑) ←→ ESP32(无线通信) ←→ 主设备
 */
...
// ESP32通信相关
#define ESP32_TX_PIN 16  // Mega2560 TX2
#define ESP32_RX_PIN 17  // Mega2560 RX2
#define ESP32_STATUS_PIN 2  // 状态指示

// 在Mega2560上直接使用Serial2
...
const unsigned long SEND_INTERVAL = 1000; // 1秒发送一次
...
void setup() {
  ...
  Serial.println("==========================================");
  Serial.println("Mega2560 + ESP32 从设备启动");
  Serial.println("==========================================");
...
void loop() {
  ...
  // 定时发送数据到主设备（当数据变化时）
  if (dataChanged && millis() - lastDataSend > SEND_INTERVAL) {
    sendDataToMaster();
    lastDataSend = millis();
    dataChanged = false;
  }
...
void initESP32Communication() {
  ...
  unsigned long timeout = millis() + 10000; // 增加超时时间到10秒
...
void handleESP32Communication() {
  if (esp32Serial.available()) {
    ...
    if (data.startsWith("MASTER_DATA:")) {
      ...
      esp32Connected = true;
      digitalWrite(ESP32_STATUS_PIN, HIGH);
      Serial.println("ESP32重新连接成功！");
    }
  }
}

// 解析主设备数据
void parseMasterData(String data) {
  // 格式: MASTER_DATA:angle,speed,ltime,rtime,mspeed,midval,show,flag,msw,ssw,msflag,enc0,enc1,enc2,enc3,enc4
  int startIndex = 12; // "MASTER_DATA:"的长度
  
  // 解析各个参数
  int commaIndex = data.indexOf(',', startIndex);
  masterData.steerAngle = data.substring(startIndex, commaIndex).toFloat();
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.steerSpeed = data.substring(startIndex, commaIndex).toInt();
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.steerLTime = data.substring(startIndex, commaIndex).toInt();
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.steerRTime = data.substring(startIndex, commaIndex).toInt();
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.motorSpeed = data.substring(startIndex, commaIndex).toInt();
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.steerMidVal = data.substring(startIndex, commaIndex).toInt();
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.steerAngle_show = data.substring(startIndex, commaIndex).toInt();
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.SteerLeftTimeFlag = data.substring(startIndex, commaIndex).toInt();
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.motorSw = data.substring(startIndex, commaIndex).toInt() == 1;
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.steerSw = data.substring(startIndex, commaIndex).toInt() == 1;
  
  startIndex = commaIndex + 1;
  commaIndex = data.indexOf(',', startIndex);
  masterData.motorSwFlag = data.substring(startIndex, commaIndex).toInt();
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (i < 4) {
      commaIndex = data.indexOf(',', startIndex);
      masterData.encoders[i] = data.substring(startIndex, commaIndex).toInt();
    } else {
      masterData.encoders[i] = data.substring(startIndex).toInt();
    }
  }
  
  masterData.lastUpdate = millis();
  masterData.valid = true;
  
  Serial.println("=== 接收到的主设备数据 ===");
  Serial.print("舵机角度: "); Serial.println(masterData.steerAngle);
  Serial.print("舵机速度: "); Serial.println(masterData.steerSpeed);
  Serial.print("左转时间: "); Serial.println(masterData.steerLTime);
  Serial.print("右转时间: "); Serial.println(masterData.steerRTime);
  Serial.print("电机速度: "); Serial.println(masterData.motorSpeed);
  Serial.print("中位值: "); Serial.println(masterData.steerMidVal);
  Serial.print("显示角度: "); Serial.println(masterData.steerAngle_show);
  Serial.print("电机开关: "); Serial.println(masterData.motorSw ? "ON" : "OFF");
  Serial.print("舵机开关: "); Serial.println(masterData.steerSw ? "ON" : "OFF");
  Serial.print("编码器数据: ");
  for (int i = 0; i < 5; i++) {
    Serial.print(masterData.encoders[i]);
    if (i < 4) Serial.print(", ");
  }
  Serial.println();
  Serial.println("========================");
  ...
  // 同步主设备数据到本地
  syncMasterDataToLocal();
}

// 同步主设备数据到本地
void syncMasterDataToLocal() {
  if (masterData.valid) {
    // 同步系统参数
    Val.steerAngle = masterData.steerAngle;
    Val.steerSpeed = masterData.steerSpeed;
    Val.steerLTime = masterData.steerLTime;
    Val.steerRTime = masterData.steerRTime;
    Val.motorSpeed = masterData.motorSpeed;
    Val.steerMidVal = masterData.steerMidVal;
    Val.steerAngle_show = masterData.steerAngle_show;
    Val.SteerLeftTimeFlag = masterData.SteerLeftTimeFlag;
    Val.motorSw = masterData.motorSw;
    Val.steerSw = masterData.steerSw;
    Val.motorSwFlag = masterData.motorSwFlag;
    
    // 同步编码器数据
    for (int i = 0; i < 5; i++) {
      ecCnt[i] = masterData.encoders[i];
    }
...
void sendDataToMaster() {
  if (esp32Connected) {
    // 构建数据包
    String dataPacket = "SLAVE_DATA:";
    dataPacket += String(Val.steerAngle) + ",";
    dataPacket += String(Val.steerSpeed) + ",";
    dataPacket += String(Val.steerLTime) + ",";
    dataPacket += String(Val.steerRTime) + ",";
    dataPacket += String(Val.motorSpeed) + ",";
    dataPacket += String(Val.steerMidVal) + ",";
    dataPacket += String(Val.steerAngle_show) + ",";
    dataPacket += String(Val.SteerLeftTimeFlag) + ",";
    dataPacket += String(Val.motorSw ? 1 : 0) + ",";
    dataPacket += String(Val.steerSw ? 1 : 0) + ",";
    dataPacket += String(Val.motorSwFlag) + ",";
    
    // 编码器数据
    for (int i = 0; i < 5; i++) {
      dataPacket += String(ecCnt[i]);
      if (i < 4) dataPacket += ",";
    }
    
    sendToESP32(dataPacket);
    
    Serial.println("发送数据到主设备: " + dataPacket);
  }
}
