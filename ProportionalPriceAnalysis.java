...
public class ProportionalPriceAnalysis {
    ...
    public static void main(String[] args) {
        System.out.println("=== 基于比例系数的房价分析 ===");
        
        int n = 中介单价.length;
        System.out.println("数据量: " + n);
        
        // 整体比例分析
        analyzeOverallProportion();
        ...
        for (int i = 0; i < 价格范围.length; i++) {
            if (分组数据[i].size() > 0) {
                analyzePriceRangeProportion(分组数据[i], 范围名称[i], i);
            }
        }
        
        // 生成综合比例公式
        generateComprehensiveProportionFormula(分组数据);
    ...
    // 整体比例分析
    private static void analyzeOverallProportion() {
        System.out.println("\n=== 整体比例分析 ===");
        
        // 计算比例系数
        double[] 比例系数 = new double[中介单价.length];
        for (int i = 0; i < 中介单价.length; i++) {
            if (国信达单价[i] > 0) {
                比例系数[i] = 中介单价[i] / 国信达单价[i];
            }
        }
        
        double 平均比例 = calculateMean(比例系数);
        double 比例标准差 = calculateStandardDeviation(比例系数);
        
        System.out.printf("整体平均比例系数: %.4f%n", 平均比例);
        System.out.printf("比例系数标准差: %.4f%n", 比例标准差);
        System.out.printf("整体公式: 中介单价 = %.4f × 国信达单价%n", 平均比例);
        
        // 计算R²
        double[] 预测值 = new double[中介单价.length];
        for (int i = 0; i < 预测值.length; i++) {
            预测值[i] = 平均比例 * 国信达单价[i];
        }
        double r2 = calculateRSquared(中介单价, 预测值, calculateMean(中介单价));
        double mae = calculateMAE(中介单价, 预测值);
        
        System.out.printf("整体R²: %.4f, MAE: %.2f%n", r2, mae);
    }
    ...
    // 分析单个价格范围的比例系数
    private static void analyzePriceRangeProportion(List<PriceRangeData> 数据, String 范围名称, int 范围索引) {
        if (数据.size() < 3) {
            System.out.println("\n" + 范围名称 + " - 数据量不足，跳过分析");
            return;
        }
        ...
        // 计算比例系数
        double[] 比例系数 = new double[中介单价数组.length];
        for (int i = 0; i < 中介单价数组.length; i++) {
            if (国信达单价数组[i] > 0) {
                比例系数[i] = 中介单价数组[i] / 国信达单价数组[i];
            }
        }
        ...
        System.out.printf("平均比例系数: %.4f%n", 平均比例);
        System.out.printf("比例系数标准差: %.4f%n", 比例标准差);
        System.out.printf("比例公式: 中介单价 = %.4f × 国信达单价%n", 平均比例);
        
        // 计算R²
        double[] 预测值 = new double[中介单价数组.length];
        for (int i = 0; i < 预测值.length; i++) {
            预测值[i] = 平均比例 * 国信达单价数组[i];
        }
        double r2 = calculateRSquared(中介单价数组, 预测值, calculateMean(中介单价数组));
        double mae = calculateMAE(中介单价数组, 预测值);
        
        System.out.printf("R²: %.4f, MAE: %.2f%n", r2, mae);
        
        // 基于总价的比例分析
        double[] 总价比例系数 = new double[中介单价数组.length];
        for (int i = 0; i < 中介单价数组.length; i++) {
            if (国信达总价数组[i] > 0) {
                总价比例系数[i] = 中介单价数组[i] / 国信达总价数组[i];
            }
        }
        
        double 平均总价比例 = calculateMean(总价比例系数);
        double 总价比例标准差 = calculateStandardDeviation(总价比例系数);
        
        System.out.printf("总价比例系数: %.6f%n", 平均总价比例);
        System.out.printf("总价比例公式: 中介单价 = %.6f × 国信达总价%n", 平均总价比例);
        
        // 计算总价R²
        double[] 总价预测值 = new double[中介单价数组.length];
        for (int i = 0; i < 总价预测值.length; i++) {
            总价预测值[i] = 平均总价比例 * 国信达总价数组[i];
        }
        double 总价r2 = calculateRSquared(中介单价数组, 总价预测值, calculateMean(中介单价数组));
        double 总价mae = calculateMAE(中介单价数组, 总价预测值);
        
        System.out.printf("总价R²: %.4f, 总价MAE: %.2f%n", 总价r2, 总价mae);
        
        // 保存范围数据
        数据.get(0).setProportionParams(平均比例, r2, mae, 平均总价比例, 总价r2, 总价mae);
    }
    
    // 生成综合比例公式
    private static void generateComprehensiveProportionFormula(List<PriceRangeData>[] 分组数据) {
        System.out.println("\n=== 综合比例公式 ===");
        
        for (int i = 0; i < 分组数据.length; i++) {
            if (分组数据[i].size() > 3) {
                PriceRangeData 样本 = 分组数据[i].get(0);
                System.out.printf("%s: 中介单价 = %.4f × 国信达单价 (R²=%.4f)%n", 
                    范围名称[i], 样本.比例系数, 样本.r2);
                System.out.printf("        或: 中介单价 = %.6f × 国信达总价 (R²=%.4f)%n", 
                    样本.总价比例系数, 样本.总价r2);
            }
        }
    }
    ...
    private static void saveResultsToFile(List<PriceRangeData>[] 分组数据) {
        try (PrintWriter writer = new PrintWriter(new FileWriter("proportional_price_formula.txt"))) {
            writer.println("=== 基于比例系数的房价分析结果 ===");
            writer.println();
            ...
            for (int i = 0; i < 分组数据.length; i++) {
                if (分组数据[i].size() > 3) {
                    ...
                    writer.printf("单价比例公式: 中介单价 = %.4f × 国信达单价 (R²=%.4f)%n", 
                        样本.比例系数, 样本.r2);
                    writer.printf("总价比例公式: 中介单价 = %.6f × 国信达总价 (R²=%.4f)%n", 
                        样本.总价比例系数, 样本.总价r2);
                    writer.println();
                }
            }
            
            System.out.println("\n结果已保存到 proportional_price_formula.txt");
        } catch (IOException e) {
    ...
    static class PriceRangeData {
        ...
        public void setProportionParams(double 比例系数, double r2, double mae,
                                      double 总价比例系数, double 总价r2, double 总价mae) {
            this.比例系数 = 比例系数;
            this.r2 = r2;
            this.mae = mae;
            this.总价比例系数 = 总价比例系数;
