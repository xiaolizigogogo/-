 ...
 * 架构：Mega2560主设备 ←→ ESP32(AP) ←→ WiFi直连 ←→ ESP32(Station) ←→ Mega2560从设备
 */
...
void setup() {
  ...
  Serial.println("==========================================");
  Serial.println("ESP32 AP模式主设备启动");
  Serial.println("==========================================");
  ...
  Serial.println("ESP32 AP主设备初始化完成！");
}
...
void syncData() {
  if (millis() - lastDataSync > 1000) { // 每1秒同步一次
    ...
    if (lastMasterData.length() > 0 && millis() - lastMasterUpdate < 5000) {
      if (apStarted && clientConnected) {
        broadcastUDPData(lastMasterData);
      }
    }
    ...
    if (lastSlaveData.length() > 0 && millis() - lastSlaveUpdate < 5000) {
      if (apStarted && clientConnected) {
        broadcastUDPData(lastSlaveData);
      }
    }
...
String getSystemStatus() {
  String status = "AP: " + String(apStarted ? "已启动" : "未启动") + 
                  ", Mega: " + String(megaConnected ? "已连接" : "未连接") + 
                  ", 客户端: " + String(clientConnected ? "已连接" : "未连接");
  return status;
}
