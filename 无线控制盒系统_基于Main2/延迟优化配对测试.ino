...
void setup() {
  ...
  Serial.println("=== 延迟优化配对测试程序 ===");
  Serial.println("解决433MHz模块发送延迟问题");
...
void loop() {
  ...
  if (pairingMode) {
    ...
    // 每3秒发送一次配对请求（增加间隔，考虑延迟）
    if (currentTime - lastPairingSendTime >= 3000) {
    ...
    if (Serial2.available()) {
      ...
      // 如果没有找到完整帧，检查是否有部分数据
      if (dataCount > 0) {
        Serial.print(">>> 收到部分数据，等待更多数据...");
        // 再次等待
        delay(200);
        if (Serial2.available()) {
          Serial.println(" 有更多数据到达");
        } else {
          Serial.println(" 没有更多数据");
        }
      }
    }
    
    // 简化配对逻辑：如果进入配对模式8秒后，假设配对成功
    static bool pairingModeEntered = false;
    if (!pairingModeEntered && currentTime - pairingStartTime > 8000) {
      Serial.println(">>> 配对模式已启动8秒，假设配对成功");
