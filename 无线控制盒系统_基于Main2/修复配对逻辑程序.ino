...
void setup() {
  ...
  Serial.println("=== 修复配对逻辑程序 ===");
  Serial.println("修复配对状态不一致和数据发送不稳定问题");
...
void loop() {
  ...
  if (digitalRead(bt1) == LOW) {
    if (!bt1_pressed) {
      ...
      if (currentTime - bt1_press_time >= 3000 && !pairingMode) {
        ...
        // 进入配对模式时不重新初始化模块，避免干扰发送功能
        Serial.println(">>> 进入配对模式，使用现有模块状态");
      }
    }
  } else {
    if (bt1_pressed) {
  ...
  if (pairingMode) {
    ...
    if (Serial2.available()) {
      ...
      // 处理接收到的数据
      processReceivedData(receivedData, dataCount, true);
    }
  } else {
    // 正常工作模式
    if (pairingState.isPaired) {
      ...
      if (Serial2.available()) {
        ...
        // 处理接收到的数据
        processReceivedData(receivedData, dataCount, false);
      }
...
// 处理接收到的数据
void processReceivedData(byte* data, int count, bool inPairingMode) {
  // 将数据添加到缓冲区
  for (int i = 0; i < count; i++) {
    if (bufferIndex < 20) {
      frameBuffer[bufferIndex] = data[i];
      bufferIndex++;
    }
  }
  ...
  for (int i = 0; i <= bufferIndex - 7; i++) {
    if (frameBuffer[i] == 0xFD && frameBuffer[i + 6] == 0xDF) {
      // 找到完整的数据帧
      Serial.print(">>> 找到完整数据帧: ");
      for (int j = 0; j < 7; j++) {
        Serial.print("0x");
        if (frameBuffer[i + j] < 0x10) Serial.print("0");
        Serial.print(frameBuffer[i + j], HEX);
        Serial.print(" ");
      }
      Serial.println();
      
      // 分析数据帧内容
      if (frameBuffer[i + 1] == 0x03) {
        if (frameBuffer[i + 5] == 0x96) {
          ...
          pairingState.lastHeartbeat = millis();
        ...
        } else if (frameBuffer[i + 5] == 0x98) {
          Serial.println(">>> 收到心跳信号");
          if (pairingState.isPaired) {
            pairingState.lastHeartbeat = millis();
          }
        } else if (frameBuffer[i + 5] == 0x99) {
          Serial.println(">>> 收到控制指令");
          // 这里可以添加具体的控制逻辑
        } else {
          Serial.print(">>> 未知数据帧，最后字节: 0x");
          Serial.println(frameBuffer[i + 5], HEX);
        }
      } else {
        Serial.print(">>> 未知命令: 0x");
        Serial.println(frameBuffer[i + 1], HEX);
      }
      
      // 移除已处理的数据帧
      for (int j = i + 7; j < bufferIndex; j++) {
        frameBuffer[j - 7] = frameBuffer[j];
      }
      bufferIndex -= 7;
      break;
    }
