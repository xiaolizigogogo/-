...
void setup() {
  ...
  Serial.println("=== 时序修复配对测试程序 ===");
  Serial.println("修复433MHz模块的发送时序和缓冲区问题");
...
void loop() {
  ...
  if (pairingMode) {
    ...
    if (currentTime - lastPairingSendTime >= 5000) {
      ...
      // 发送配对请求：使用修复的时序
      Serial.println(">>> 发送配对请求：FD 0E 00 00 00 00 DF");
      
      Serial2.write((byte)0xFD);  // 起始位
      delay(2);  // 关键：添加延迟
      Serial2.write((byte)0x0E);  // 命令
      delay(2);  // 关键：添加延迟
      Serial2.write((byte)0x00);  // 数据1
      delay(2);  // 关键：添加延迟
      Serial2.write((byte)0x00);  // 数据2
      delay(2);  // 关键：添加延迟
      Serial2.write((byte)0x00);  // 数据3
      delay(2);  // 关键：添加延迟
      Serial2.write((byte)0x00);  // 数据4
      delay(2);  // 关键：添加延迟
      Serial2.write((byte)0xDF);  // 结束位
      Serial2.flush();
      ...
      for (int i = 0; i <= dataCount - 7; i++) {
        if (receivedData[i] == 0xFD && receivedData[i + 6] == 0xDF) {
          ...
          if (receivedData[i + 1] == 0x0E) {
            ...
            Serial2.write((byte)0xFD);  // 起始位
            delay(2);
            Serial2.write((byte)0x0F);  // 配对确认命令
            delay(2);
            Serial2.write((byte)0x00);  // 数据1
            delay(2);
            Serial2.write((byte)0x00);  // 数据2
    ...
    if (!pairingModeEntered && currentTime - pairingStartTime > 15000) {
      ...
      Serial.print(">>> 错误统计: 共检测到");
      Serial.print(errorCount);
      Serial.println("个F0错误代码");
