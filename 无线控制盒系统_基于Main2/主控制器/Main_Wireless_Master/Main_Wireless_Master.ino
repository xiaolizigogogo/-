/*
 * Mega2560 + ESP32 主设备 - 基于Main(2).ino的完整业务逻辑
 * 功能：保持原始Main功能 + 通过ESP32向从设备同步数据
 * 架构：Mega2560(业务逻辑) ←→ ESP32(无线通信) ←→ 从设备
 */
...
void setup() {
  ...
  Serial.println("==========================================");
  Serial.println("Mega2560 + ESP32 主设备启动");
  Serial.println("==========================================");
...
void parseSlaveStatus(String data) {
  // 格式: SLAVE_STATUS:connected,ready,count,time
  int firstComma = data.indexOf(',', 13);
  int secondComma = data.indexOf(',', firstComma + 1);
  int thirdComma = data.indexOf(',', secondComma + 1);
  
  if (firstComma > 0 && secondComma > 0 && thirdComma > 0) {
    slaveStatus.connected = data.substring(13, firstComma).toInt() == 1;
    slaveStatus.systemReady = data.substring(firstComma + 1, secondComma).toInt() == 1;
    slaveStatus.executionCount = data.substring(secondComma + 1, thirdComma).toInt();
    slaveStatus.lastUpdate = millis();
    
    Serial.println("从设备状态 - 连接:" + String(slaveStatus.connected) + 
                   ", 就绪:" + String(slaveStatus.systemReady) + 
                   ", 执行次数:" + String(slaveStatus.executionCount));
  }
}

// 解析从设备数据
void parseSlaveData(String data) {
  // 格式: SLAVE_DATA:sensor,relay,led,motor
  int firstComma = data.indexOf(',', 11);
  int secondComma = data.indexOf(',', firstComma + 1);
  int thirdComma = data.indexOf(',', secondComma + 1);
  
  if (firstComma > 0 && secondComma > 0 && thirdComma > 0) {
    slaveStatus.sensorValue = data.substring(11, firstComma).toInt();
    slaveStatus.relayState = data.substring(firstComma + 1, secondComma).toInt() == 1;
    slaveStatus.ledState = data.substring(secondComma + 1, thirdComma).toInt() == 1;
    slaveStatus.motorSpeed = data.substring(thirdComma + 1).toInt();
    
    Serial.println("从设备数据 - 传感器:" + String(slaveStatus.sensorValue) + 
                   ", 继电器:" + String(slaveStatus.relayState ? "ON" : "OFF") + 
                   ", LED:" + String(slaveStatus.ledState ? "ON" : "OFF") + 
                   ", 电机:" + String(slaveStatus.motorSpeed));
  }
}

// 同步数据到从设备
void syncDataToSlave() {
  if (esp32Connected && dataChanged) {
    // 构建数据包
    String dataPacket = "MASTER_DATA:";
    ...
    Serial.println("发送数据到从设备: " + dataPacket);
  }
}
