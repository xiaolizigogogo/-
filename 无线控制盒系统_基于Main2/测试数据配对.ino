...
void setup() {
  ...
  Serial.println("=== 测试数据配对程序 ===");
  Serial.println("使用成功的测试数据格式进行配对");
...
void loop() {
  ...
  if (pairingMode) {
    ...
    if (currentTime - lastPairingSendTime >= 5000) {
      ...
      delay(2);
      Serial2.write((byte)0x03);  // 命令（使用成功的命令）
      delay(2);
      Serial2.write((byte)0xD5);  // 数据1
      delay(2);
      Serial2.write((byte)0xAA);  // 数据2
      delay(2);
      Serial2.write((byte)0x55);  // 数据3
      delay(2);
      Serial2.write((byte)0x96);  // 数据4
      delay(2);
      ...
      for (int i = 0; i <= dataCount - 7; i++) {
        if (receivedData[i] == 0xFD && receivedData[i + 6] == 0xDF) {
          ...
          if (receivedData[i + 1] == 0x03) {
            ...
            Serial2.write((byte)0x55);  // 数据3
            delay(2);
            Serial2.write((byte)0x97);  // 数据4（使用不同的值表示确认）
            delay(2);
            Serial2.write((byte)0xDF);  // 结束位
            Serial2.flush();
