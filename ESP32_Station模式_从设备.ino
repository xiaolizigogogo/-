/*
 * ESP32 Station模式从设备 - 桥接模块
 * 功能：连接到AP热点，连接Mega2560从设备，实现无线数据同步
 * 架构：Mega2560从设备 ←→ ESP32(Station) ←→ WiFi直连 ←→ ESP32(AP) ←→ Mega2560主设备
 */
...
void setup() {
  ...
  Serial.println("==========================================");
  Serial.println("ESP32 Station模式从设备启动");
  Serial.println("==========================================");
  ...
  Serial.println("ESP32 Station从设备初始化完成！");
}
...
String getSystemStatus() {
  String status = "WiFi: " + String(wifiConnected ? "已连接" : "未连接") + 
                  ...
                  ", 服务器: " + String(serverConnected ? "已连接" : "未连接");
  return status;
}
